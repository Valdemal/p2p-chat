...
class ChatForm(npyscreen.FormBaseNew):
    def create(self):
        ...
        self.chatFeed = self.add(npyscreen.BoxTitle, name=LANG['interface']['feed'], editable=False,
                                 max_height=self.y - 7)
        self.chatInput = self.add(ChatInput, name=LANG['interface']['input'], footer=LANG['interface']['footer'],
                                  rely=self.y - 5)
